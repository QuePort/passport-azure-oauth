...
function Strategy(options, verify) {
  ...
  this._verify = verify;
	...
	this.setOptionsWithDefaults(options);
...
>>>>>>> 1f4ac5f... DEV001 - fixed Azure login -> username for auto fill of input field is now accessed by getParam 'login_hint'
}
...
Strategy.prototype.setOptionsWithDefaults = function(options) {
  ...
  if (options.authRelativeURL) {
    ...
    this._authRelativeURL = '/oauth2' + version + 'authorize';
  }

  if (options.tokenRelativeURL) {
    this._tokenRelativeURL = options.tokenRelativeURL;
  } else {
    this._tokenRelativeURL = '/oauth2' + version + 'token';
  }
  if (options.redirectUrl) {
    this.redirectUrl = options.redirectUrl;
  }
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  console.log('strategy.js:OAUTH authenticate called');

  // check if there is a app token present
  if (typeof this._refreshToken != 'undefined') {
  console.log('strategy.js: refreshToken != undefiend ');
    ...
    this._oauth2.getOAuthAccessToken(
        this._refreshToken,
        {grant_type: 'refresh_token', refresh_token: this._refreshToken, resource: this._resource, redirect_uri : this.redirectUrl },
  ...
  console.log('strategy.js: req != undefiend && req.query && req.query.code='+req.query.code);
    this._oauth2    = new OAuth2(this._clientId, this._clientSecret, '', this._authURL, this._tokenURL, '', this.proxy);
    ...
    console.log('strategy.js: about to call getOAUTHAccessToken requestParams='+JSON.stringify(requestParams));
    ...
    this._oauth2.getOAuthAccessToken(code, requestParams,
      function(err, accessToken, refreshToken, params) {
        console.log('strategy.js: getOAuthAccessToken callback. err='+err);
        if (err) { return self.error(new InternalOAuthError('failed to obtain access token', err)); }

        console.log('strategy.js: calling self._loadUserProfile accessToken='+accessToken);
        self._loadUserProfile(accessToken, refreshToken, function(err, profile) {
          console.log('strategy.js: calling callback. err='+err);
          console.log('strategy.js: calling callback. profile='+JSON.stringify(profile));
          if (err) { return self.error(err); };
    ...
    console.log('strategy.js: create new OAuth2 object.');
    this._oauth2 = new OAuth2(this._clientId,  this._clientSecret, '', this._authURL, this._tokenURL, '', this.proxy);
    ...
    if (options.policy) {
      location += '&scope=openid+offline_access';
    }

    console.log('strategy.js: redirecting to location='+location);
...
Strategy.prototype.userProfile = function(accessToken, refreshToken, done) {
	...
	try {
		console.log(`strategy.js: Called prototype userProfile()`);
  ...
  if(this._resource === "https://api.office.com/discovery/"){
    ...
    //get all service endpoints
		console.log('strategy.js: calling sendRestRequest options='+JSON.stringify(options));
    sendRestRequest(options, function(err, services) {
      if (services) {
		...
		console.log('strategy.js: this._resource !== api.office.com/discovery. calling done. profile='+JSON.stringify(profile));
    done(null, profile);
  }
};
...
var sendRestRequest = function(options, callback) {
  var req = https.get(options, function(res) {
    console.log('sendRestRequest callback called. res='+res);
    res.setEncoding('utf8');
    var result = '';
    
    res.on('data', function(data) {
      console.log('sendRestRequest data event. data='+data);
      result += data;
    });
    
    res.on('end', function() {
      console.log('sendRestRequest end event. result='+result);
      try {
    ...
    console.log('sendRestRequest error event. e='+e);
...
Strategy.prototype.authorizationParamsForUrl = function(options) {
  var params = '';
  for (var attr in options) {
    if (attr != 'clientId' && attr != 'clientSecret' && attr != 'resource' && attr != 'tenantId' && attr != 'successRedirect' && attr !='failureRedirect') {
...
Strategy.prototype._loadUserProfile = function(accessToken, refreshToken, done) {
  ...
  function loadIt() {
    console.log('strategy.js: called loadIt(). calling self.userProfile()');
    return self.userProfile(accessToken, refreshToken, done);
  }
  function skipIt() {
    console.log('strategy.js: called skipIt()');
    return done(null);
  }

  if (typeof this._skipUserProfile == 'function' && this._skipUserProfile.length > 1) {
    console.log('strategy.js: skipUserProfile == function. calling it');
    // async
    ...
    console.log('strategy.js: skipUserProfile not function or length not > 1');
