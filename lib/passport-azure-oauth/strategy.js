...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (typeof this._refreshToken != 'undefined') {
    ...
    this._oauth2 = new OAuth2(this._clientId,  this._clientSecret, '', this._authURL, this._tokenURL);
    this._oauth2.getOAuthAccessToken(
        this._refreshToken,
        {grant_type: 'refresh_token', refresh_token: this._refreshToken, resource: this._resource},
    ...
    this._oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', resource: this._resource },
      function(err, accessToken, refreshToken, params) {
        if (err) { return self.error(new InternalOAuthError('failed to obtain access token', err)); }
