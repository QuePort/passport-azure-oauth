    ...
    sendRestRequest(options, function(err, services) {
      if (services) {
        async.each(services, function(item, callback) {
          ...
          if (profile.endpoints["Directory@AZURE"]) {
            isUserPermittedForApp(profile.endpoints["Directory@AZURE"], profile, function(err, isPermitted) {
              if (isPermitted) {
                done(null, profile);
              } else {
                returnNoAccessMessage(profile, '', done);
              }
            });
          } else {
            returnNoAccessMessage(profile, '', done);
        ...
        returnNoAccessMessage(profile, (err ? err.message : ''), done);
      }
    });
  } else {
    done(null, profile);
...
//Returns the current user profile with an error message, that the current user has no access to the current application.
var returnNoAccessMessage = function(profile, message, callback) {
  var error = new Error();
  error.message = 'User has no access to any Services!',
  error.code    = 401;
